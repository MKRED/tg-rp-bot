 ...
 * запросе — как у `knowledge_books b`) по sort_order записей с привязанным character/persona
 * (свободнотекстовые записи в стек не идут — им нечего показать). json_agg схлопывает N строк
  ...
  return sql`
    LEFT JOIN LATERAL (
      ...
      FROM (
        ...
        WHERE e.book_id = b.id AND (e.character_id IS NOT NULL OR e.persona_id IS NOT NULL)
