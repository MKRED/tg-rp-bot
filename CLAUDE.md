...
**ВСЕГДА** вызывай субагента **`advisor`** (Agent tool, `subagent_type: advisor`, модель opus) в трёх
типах моментов — аналог триггеров встроенного advisor tool Claude Code. Он консультирует и рассуждает,
код не правит.

1. **Перед тем как закоммититься к подходу** — выбор подхода/архитектуры, сомнение между вариантами,
   неочевидное решение, оценка компромиссов, «как правильно сделать X».
2. **Когда застрял на повторяющейся ошибке.** Если **две** попытки подряд починить один и тот же симптом
   (тот же текст ошибки / тот же падающий тест / то же неверное поведение) не помогли — третью попытку
   НЕ начинаем, сперва спрашиваем advisor. Правки в разных местах — это попытки по тому же симптому,
   если симптом не изменился. НЕ считаются попытками: перезапуск известных флейков и правка, после
   которой симптом реально изменился (счётчик обнуляется). Advisor тут даёт не очередной фикс, а
   проверку модели проблемы («чинишь не то место / неверная предпосылка») и может отправить к
   `bug-investigator` за полноценной трассировкой.
3. **Перед тем как объявить задачу выполненной** — для нетривиальных задач (критерий ниже), как
   независимая проверка **до** `code-reviewer` и коммита (если подход неверен, построчное ревью диффа
   бессмысленно). Вопрос advisor'у здесь не про стиль диффа, а: решает ли сделанное то, что реально
   просил пользователь; всё ли из просьбы закрыто; верен ли был выбранный подход; какие edge cases и
   последствия за пределами диффа (прод-данные, миграции, инварианты, совместимость) упущены. Повторно
   на этом шаге зовём, только если по итогам менялся подход.

**Нетривиальная задача** (для триггера 3, достаточно одного пункта): затронуто ≥3 файла; затронуты оба
пакета (`bot/` + `webapp/`); есть миграция/изменение схемы БД; задет инвариант или граница (прокси,
BYOK-ключ, Mini App boundary/initData, narrator/compact, SSE-стрим); по ходу менялся подход или
что-то откатывалось; задача поставлена целью («хочу, чтобы…»), а не точной правкой. Мелкие точечные
правки (один файл, текст/косметика, буквально продиктованная правка) — advisor на шаге 3 не нужен.

**Advisor стартует с ЧИСТЫМ контекстом** — в отличие от встроенного advisor tool он не видит переписку
и вывод инструментов автоматически. Всё нужное передаём в prompt Agent-вызова явно, отсылки «как
обсуждали выше» бесполезны. Минимальный пакет по триггерам:
- **(1) подход:** цель дословно; рассматриваемые варианты и текущая ставка; пути релевантных файлов;
  ограничения (прод-БД без стейджа, инварианты); что уже отверг и почему.
- **(2) застрял:** точный текст ошибки/симптома; хронология попыток «что менял → что получил»; текущая
  гипотеза; пути правленых файлов; что ещё не проверял.
- **(3) готовность:** дословная формулировка задачи пользователем + уточнения по ходу; что сделано
  (файлы + суть правок); чего сознательно не делал; допущения и известные риски; статус тестов/сборки.

Не дублируем: `code-reviewer` — конвенции и корректность диффа; `test-runner` — гейт тестов+сборки;
`bug-investigator` — поиск корня конкретного бага. Advisor — про подход, модель проблемы и соответствие
результата задаче. Не полагайся только на собственное суждение в сложных моментах — сначала спроси
совета у `advisor`.
...
|---|---|---|
| **`advisor`** | Подход/архитектура · застревание на ошибке · проверка перед «готово» (см. выше — **mandatory**, 3 триггера) | нет |
